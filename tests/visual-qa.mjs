...
for (const viewport of [
  ...
  assert.equal(homeMetrics.bodyText.includes('↗'), false, `${viewport.name}: literal emoji-capable arrow is visible`);
  ...
  await page.goto(`${baseUrl}/kauzatsiya/`, { waitUntil: 'networkidle' });
  const releases = page.locator('.work-page__links li');
  assert.equal(await releases.count(), 7, `${viewport.name}: Kauzatsiya release count`);
  assert.match(await releases.first().innerText(), /От меня до тебя\s*album/i);
  assert.ok(await releases.first().locator('.release-label').isVisible(), `${viewport.name}: album stamp is visible`);
  assert.ok(await page.locator('.external-icon').first().isVisible(), `${viewport.name}: CSS external icon is visible`);
  ...
  await page.goto(`${baseUrl}/aotrom/`, { waitUntil: 'networkidle' });
